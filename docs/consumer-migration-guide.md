...
**Pattern 1 requires a `*.l42.eu` domain.** In-app verification reads the shared **`aithne_session`** cookie, which aithne sets in production with `Domain=l42.eu` — so the browser only sends it to `l42.eu` and its subdomains. A consumer served from any other domain never receives the cookie and cannot use this pattern (the canonical `lucas42/lucos_arachne` middleware reads `aithne_session` straight off the request; aithne's cookie domain is set in `token/token.go`). Patterns 2 and 3 are **not** bound by this: each runs a standard OAuth2/OIDC redirect flow and sets its *own* session cookie (pattern 3 via oauth2-proxy's own cookie), so they work on any domain, subject only to registering the redirect URI with aithne.
