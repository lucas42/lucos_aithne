...
A fresh session resolves to a **near-empty default-deny scope** — it authenticates fine but can do almost nothing until capability is granted one named scope at a time. Backends have two complementary levers: **scope-based** (capabilities; the only safe lever for non-human principals) and **identity-based** (gate on contact-id for per-resource ownership aithne need not know about). **A sensitive action must never be gated on bare "is there a valid session?"** — a machine principal would pass; gate on scope or contact-id. **Scope is the *sole* authorisation gate:** consumers MUST NOT maintain a hardcoded `principal_class` allowlist or reject a token for an absent/unrecognised class — default-deny already prevents any class, existing or future, from reaching a resource without an explicit grant, so an allowlist only guards a door scope already locks while risking an estate-wide flag-day when aithne adds a class (see the [local verification contract](../local-verification-contract.md) §5). `principal_class` stays significant only for *interpreting* `sub` (identity-scoped access), never as an access-control gate.
